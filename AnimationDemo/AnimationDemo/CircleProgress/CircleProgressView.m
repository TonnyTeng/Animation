...
- (void)initialization {
    ...
    _pathBackColor = [UIColor colorWithRed:204 green:204 blue:204 alpha:1];//RGB(204, 204, 204);
    _pathFillColor = [UIColor colorWithRed:219 green:184 blue:102 alpha:1];//RGB(219, 184, 102);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showPoint) {
        ...
        CGContextDrawImage(ctx, CGRectMake(self.frame.size.width / 2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*cosf(valueEndA)-_strokeWidth/2.0, self.frame.size.width / 2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*sinf(valueEndA)-_strokeWidth/2.0, _strokeWidth, _strokeWidth), _pointImage.CGImage);
    }
    ...
    if (_showProgressText) {
        ...
        //字体
        UIFont *font = [UIFont systemFontOfSize:0.15 * self.frame.size.width];
        //构建属性集合
        ...
        //垂直居中
        CGRect r = CGRectMake((self.frame.size.width - stringSize.width)/2.0, (self.frame.size.height - stringSize.height)/2.0,stringSize.width, stringSize.height);
...
- (void)setProgress:(CGFloat)progress {
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:0.005 repeats:YES block:^(NSTimer * _Nonnull timer){
